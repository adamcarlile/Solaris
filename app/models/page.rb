...
#  named_scope_with_exact_match :with_parent, :parent_id
  ...
  has_many :panels, :as => :editable, :dependent => :delete_all
  accepts_nested_attributes_for :panels, :allow_destroy => true
  validates_associated :panels
  ...
  self.allowed_child_types = [:basic_page,:folder,:news_index,:hyperlink,:redirect, :faq_index, :profile_index, :list_index, :gallery, :downloads, :video_index]
  self.archive             = false
