module EasyFormsHelper
  ...
  def field(model, method, options = {})
    ...
    prompt_html = content_tag('span', options[:prompt], :class => 'prompt') if options[:prompt]
    if options[:helper] == :check_box
      html = "#{field_html} <div class='label'>#{label_html} #{prompt_html}</div> #{error_html}"
    else
      html = "<div class='label'>#{label_html} #{prompt_html}</div><br />#{field_html} #{error_html}"
    end
    content_tag('div', html, :class => (field_has_error?(model,method) ? 'formElm withError' : 'formElm'))
  end
